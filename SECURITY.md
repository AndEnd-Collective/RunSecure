...
13. **`_diag/` and `_diag-proxy/` host-mounted volumes.** As of this release, the runner's `_diag/` directory is host-mounted at the orchestrator's working directory for operator-side log recovery. Workflows that echo secrets to stdout/stderr (`set -x`, debug-print of `$DATABASE_URL`) leave those secrets in `_diag/Worker_*.log` until rotation (one previous run is kept). On shared CI hosts, `RUNSECURE_DIAG_RETENTION=0` disables the bind mount, but the synchronous wait proves only local queue drain and cannot guarantee GitHub persisted the job log. Disabling retention therefore removes the local recovery copy when remote delivery fails; verify the job-log API independently.
...
- **GitHub's Azure Blob wildcard is also an exfiltration path.** [Self-hosted runners require `*.blob.core.windows.net`](https://docs.github.com/en/actions/reference/runners/self-hosted-runners) for job logs, summaries, artifacts, and caches, so RunSecure permits that suffix in its base Squid ACL. Squid's HTTPS CONNECT policy cannot distinguish GitHub-selected storage accounts from an attacker-owned Azure account or inspect the encrypted payload. Treat the domain allowlist as destination reduction, not DLP: do not expose secrets to untrusted workflows, keep GitHub token permissions minimal, use protected environments for sensitive credentials, and inspect proxy destination logs during incident response.
