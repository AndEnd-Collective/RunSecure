...
OUTPUT=$("$RUN_SCRIPT" --unknown-flag 2>&1 || true)
EXIT_CODE=$?
...
# Test the sed logic used in run.sh: "owner/my-repo" → "rs-my-repo"
derive_name() {
    echo "$1" | sed 's|.*/||; s/[^a-zA-Z0-9_-]/-/g' | tr '[:upper:]' '[:lower:]'
}

RESULT=$(derive_name "owner/my-repo")
...
RESULT=$(derive_name "NaorPenso/DataCentric")
...
RESULT=$(derive_name "org/repo.with.dots")
...
# Test yq default extraction (same as run.sh does)
